...
Works on bf16, fp8 and mxfp4 traces: the GEMM census splits kernels by precision
(mxfp4 / fp8 / bf16 / fp16 / ...) detected from the kernel name, so the same
analyzer reports any run. MXFP4 GEMMs on ROCm are the aiter triton kernels
(``gemm_afp4wfp4`` / ``fused_gemm_afp4wfp4_split_cat``), not Tensile/CK, so the
census recognizes those too.
...
LAYER_PATTERNS = [
    ...
    ("Linear / GEMM",
        [r"^Cijk_", r"rocblas", r"hipblas",
         ...
         # aiter MXFP4 triton GEMMs (gemm_afp4wfp4, fused_gemm_afp4wfp4_split_cat)
         r"afp4wfp4", r"gemm_afp4", r"_gemm_a\w*fp4"]),
    ("RMSNorm / LayerNorm",
...
def _is_gemm(name):
    # Tensile (rocBLAS) Cijk_*, CK ck::*gemm*, OR aiter MXFP4 triton GEMMs.
    # The MXFP4 Linear path on ROCm runs aiter triton kernels (gemm_afp4wfp4 /
    # fused_gemm_afp4wfp4_split_cat) instead of Tensile/CK, so match those by
    # the afp4wfp4 / gemm_afp4 dtype token.
    return (
        ("Cijk_" in name)
        or ("ck::" in name and "gemm" in name.lower())
        or re.search(r"afp4wfp4|gemm_afp4|_gemm_a\w*fp4", name, re.IGNORECASE) is not None
    )


def _gemm_precision(name):
    # mxfp4 markers first: aiter fp4 GEMMs carry afp4wfp4 / a*fp4 / e2m1 tokens.
    if re.search(r"afp4wfp4|mxfp4|_a\w*fp4|\bfp4\b|e2m1", name, re.IGNORECASE):
        return "mxfp4"
    # fp8 markers (Tensile F8BS, or any fp8 dtype token in the name).
...
def gemm_census(events, label=""):
    ...
    print("  (GEMM = Tensile 'Cijk_*', 'ck::*gemm*', or aiter mxfp4 '*afp4wfp4*'; every one is bucketed below)")
    print()
    print("-- by precision (robust quantized-vs-bf16 answer; phase-independent) --")
