...
from sglang.srt.utils.common import is_npu, is_mps, use_intel_amx_backend
...
class MLXLogitsProcessor(nn.Module):
    def __init__(
        self,
        config,
    ...
    def _get_pruned_states(
        ...
        if (
            ...
            if logits_metadata.padded_static_len < 0:
                last_index = mx.cumsum(logits_metadata.extend_seq_lens, axis=0) - 1
            else:
    ...
    def _get_logits(
        ...
        if self.final_logit_softcapping:
            if not _is_npu and not _is_mps:
