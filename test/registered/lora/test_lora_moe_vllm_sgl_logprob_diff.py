...
class TestMoELoraRegression(unittest.TestCase):
    ...
    def test_sglang_moe_parity_strict(self):
        ...
        print("\n" + "="*140)
        print(f"{'ID':<4} | {'Max Diff':<12} | {'Mean Diff':<12} | {'Status':<8} | {'Prompt'}")
        print("-" * 140)
        ...
        for i, prompt in enumerate(PROMPTS):
            ...
            s_lps = [float(token[0]) if isinstance(token, list) else float(token) for token in s_lps_raw]
            ...
            self.assertLessEqual(actual_max, ref["max"] + eps, f"Max LogProb Diff exceeded on prompt {i}")
            self.assertLessEqual(actual_mean, ref["mean"] + eps, f"Mean LogProb Diff exceeded on prompt {i}")

            print(f"{i:<4} | {actual_max:<12.6f} | {actual_mean:<12.6f} | {'✅ PASS':<8} | {prompt}")
